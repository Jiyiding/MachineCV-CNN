...
def get_files(file_dir, N):
    ...
    if(N==0):
        ...
        temp = np.array([AoKeng, lable_Aokeng])
        # all_train转置并打乱顺序
        temp = temp.transpose()
        ...
        temp = np.array([LieWen, lable_LieWen])
        # all_train转置并打乱顺序
        temp = temp.transpose()
        np.random.shuffle(temp)
    elif(N==2):
        ...
        temp = np.array([WeiHantou, lable_WeiHantou])
        # all_train转置并打乱顺序
        temp = temp.transpose()
        np.random.shuffle(temp)
    elif (N == 3):
        ...
        temp = np.array([WeiRonghe, lable_WeiRonghe])
        # all_train转置并打乱顺序
        temp = temp.transpose()
        np.random.shuffle(temp)
    elif (N == 4):
        ...
        temp = np.array([WuQuexian, lable_WuQuexian])
        # all_train转置并打乱顺序
        temp = temp.transpose()
        np.random.shuffle(temp)
    all_moi_list = list(temp[:, 0])
    all_lables_list = list(temp[:, 1])
    all_lables_list = [int(float(i)) for i in all_lables_list]
    return all_moi_list, all_lables_list
'''
    # 将各样本和标签列表组合成总的数组形式
